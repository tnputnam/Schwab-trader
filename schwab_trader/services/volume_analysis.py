...
class VolumeAnalysisService:
    ...
    def calculate_volume_baseline(self, volumes: List[float]) -> float:
        ...
        if not volumes:
            logger.warning("No volume data provided for baseline calculation")
            return 0.0
        
        # Use EMA to give more weight to recent volumes
        ema = pd.Series(volumes).ewm(span=self.lookback_days, adjust=False).mean()
        baseline = ema.iloc[-1]
        logger.info(f"Calculated volume baseline: {baseline:,.0f}")
        return baseline
    ...
    def analyze_volume_pattern(self, symbol: str, volumes: List[float]) -> Dict:
        ...
        if len(volumes) < self.lookback_days:
            logger.warning(f"Insufficient data for {symbol}. Need {self.lookback_days} days, got {len(volumes)}")
            return {
        ...
        logger.info(f"Volume analysis for {symbol}: {analysis}")
        return analysis
    ...
    def get_volume_alerts(self, symbol: str) -> List[str]:
        ...
        if analysis['details']['unusual_activity']:
            alerts.append(f"Unusual volume activity detected: {analysis['current_volume']:,.0f} shares ({analysis['volume_ratio']:.2f}x baseline)")
        
        if analysis['details']['volume_momentum'] > 0.2:
            alerts.append(f"Strong volume momentum: {analysis['details']['volume_momentum']:.1%} increase in last 5 days")
